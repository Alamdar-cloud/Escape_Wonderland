...
pygame.init()
mixer.init()  # Initialize the mixer for music/sound playback
...
# Load images
background_image = pygame.image.load("Wonderland_background.jpeg")
...
# Load additional images for level 2 (final boss and nuke)
...
# Alice's velocity
velocity_x = 0
velocity_y = -3  # Constant upward movement

# Bullet list
bullets = []

# Boss card settings for level 2
cards = []
nuke_active = False
cards_destroyed = 0
total_cards = 20  # Total cards to dodge in level 2

# Button class for rendering and detecting clicks
class Button:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if try_again_button.is_clicked():
                    waiting_for_input = False  # Stop waiting for input and restart the game
...
def draw_game():
    ...
    # Draw the Queen of Hearts if in level 2
    if level == 2:
        screen.blit(queen_image, (screen_width // 2 - queen_image.get_width() // 2, 50))

    # Draw score and keys collected
...
# Create a random enemy card
def create_card():
    card_x = random.randint(0, screen_width - 40)
    card_y = -60  # Start the entity above the screen
    ...
    for bullet in bullets[:]:
        for card in cards[:]:
            if bullet.colliderect(card):
                ...
                cards.remove(card)  # Remove the enemy entity
                score += 50  # Score for eliminating the entity
                cards_destroyed += 1
                break  # Stop checking this bullet after it hits
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if start_button.is_clicked():
                    ...
                    menu_running = False  # Quit the game
                    pygame.quit()
                    sys.exit()
...
def game_loop():
    ...
    while running:
        ...
        score = int(elapsed_time) + keys_collected * 10  # Score based on time + collected keys
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE and level == 2 and nuke_active:
                    # Activate the nuke to clear all cards
                    cards.clear()
                    nuke_active = False  # Deactivate the nuke after use
                    score += 500  # Add score for using the nuke

        # Handle key presses for sliding
        ...
        # Prevent Alice from going off-screen
    ...
    # Stop music and show game over screen
    mixer.music.stop()
    draw_game_over()
    pygame.display.update()
